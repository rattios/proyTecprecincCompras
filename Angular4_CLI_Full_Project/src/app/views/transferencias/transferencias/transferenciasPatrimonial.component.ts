...
@Component({
  templateUrl: 'transferenciasPatrimonial.component.html'
})
export class transferenciasPatrimonialComponent {
  public prov: any;
  ...
  constructor(private http: HttpClient, private ruta: RutaService) {
   ...
   ngOnInit(): void {
      ...
      this.http.get(this.ruta.get_ruta()+'transferencias/index/patrimonial?inicio='+datePipe.transform(principio, 'dd-MM-yyyy')+'&fin='+datePipe.transform(new Date(), 'dd-MM-yyyy'))
           .toPromise()
           .then(
           data => {
             console.log(data);
              ...
              if(this.transferencias.length>0) {
                for (var i = 0; i < this.transferencias.length; i++) {
                  if(this.transferencias[i].almacen==1) {
                    this.transferencias[i].nombreAlmacenDestino=this.transferencias[i].receptor.nombre;
                    this.transferencias[i].nombreAlmacen='Almacen Principal';
                  }else if(this.transferencias[i].almacen==2){
                    this.transferencias[i].nombreAlmacenDestino=this.transferencias[i].receptor.nombre;
                    this.transferencias[i].nombreAlmacen='Almacen Secundario';
                  }
     ...
     buscar(){
      ...
      var datePipe = new DatePipe("en-US");
      this.http.get(this.ruta.get_ruta()+'transferencias/index/patrimonial?inicio='+datePipe.transform(this.inicioD2, 'dd-MM-yyyy')+'&fin='+datePipe.transform(this.finD2, 'dd-MM-yyyy'))
           .toPromise()
           .then(
           data => {
             console.log(data);
