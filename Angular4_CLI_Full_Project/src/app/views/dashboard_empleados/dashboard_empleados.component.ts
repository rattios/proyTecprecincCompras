...
@Component({
  templateUrl: 'dashboard_empleados.component.html'
})
export class Dashboard_empleadosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get(this.ruta.get_ruta()+'login/check?token='+localStorage.getItem('tecprecinc_token'))
         ...
         msg => { 
           ...
           //this.loading=false;
           alert('Se ha vencido la sesión...');
         });

    this.http.get(this.ruta.get_ruta()+'dashboard/'+localStorage.getItem('tecprecinc_usuario_id'))
         .toPromise()
         .then(
         data => {
           this.datos=data;
