...
export class reportesComponent {
  ...
  constructor(private http: HttpClient, private ruta: RutaService, private router: Router
   ...
   ngOnInit(): void {
    ...
    add(){
      console.log(this.centroCostos_id);
      if(!this.checkCentroCostos(this.centroCostos_id)) {
        for (var j = 0; j < this.cc.length; j++) {
          if(this.centroCostos_id==this.cc[j].id) {
            this.relaciones.push(this.cc[j]);
    ...
    checkCentroCostos(centroCostos_id){
      console.log(centroCostos_id);
      var band=false;
      for (var i = 0; i < this.relaciones.length; i++) {
        if(centroCostos_id==this.relaciones[i].id) {
          band=true;
        }
      }
      return band;
    ...
    crearContrato(){
     ...
     // console.log(vecRelaciones);
      var send={
          ...
          centro_costos:JSON.stringify(this.relaciones)
        };
      console.log(send);
    ...
    editarContrato(){
      ...
      this.http.put(this.ruta.get_ruta()+'relaciones/'+this.centroCosto.id,send)
           .toPromise()
           .then(
           data => {
